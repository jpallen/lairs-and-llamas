...
- **DC 16 Investigation (the floor):** A concealed floor safe is hidden beneath a flagstone under one of the workbenches. The flagstone has been recently mortared but not well — scraping away the mortar reveals a heavy iron box set into the floor. The safe is protected by an alchemical combination lock — three rotating dials, each showing an alchemical symbol (flame, droplet, crystal, leaf, skull, spiral). Clues to the combination are scattered across the lab notes: a DC 12 Investigation finds the first symbol circled in a recipe margin (flame), DC 13 finds the second referenced in an ingredient list (skull), and DC 14 finds the third encoded in a delivery schedule (crystal). Getting all three opens the safe without tools. Getting 2 of 3 reduces the Thieves' Tools DC from 15 to 10. Without any symbols, the lock requires DC 15 Thieves' Tools to pick, or DC 18 Strength to pry the whole box loose.
